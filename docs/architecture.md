...
Action::CreateLog(l)      — persist and open new log l; `LogManager::create_log` checks for duplicates (same callsign + operator + grid square on the same UTC day) before saving and returns `StorageError::DuplicateLog` if found; the error is displayed inline on the LogCreate screen
