...
4. Run the `simplify` skill on the changed files — fix any issues found before staging
5. Run `git log --oneline -5` to see recent commit message style
6. Stage relevant files (never stage `.env`, credentials, or secrets)
7. Draft a concise commit message: focus on "why" not "what"
8. Commit with trailer:
   ```
